...
def test(mode = 'test'):
    ...
    dataloader = get_dataloader(
        datapath, opt.len_closeness, opt.len_period, opt.len_trend, opt.scaler_X, opt.scaler_Y,1,False,
        mode,map_H=opt.map_height,map_W=opt.map_width,day_len=24,channel=opt.nb_flow)
...
if __name__ == '__main__':
    save_path = 'test_result/to_stage/no_ext(f+r)'
    # save_path = 'test_result/NoSpa/no_ext(r)'
    os.makedirs(save_path, exist_ok=True)
    f = open('{}/results.txt'.format(save_path), 'a')
    f.write('=' * 50)
    f.write('\n')
